...
import com.dev.prepcarapplication.fragment.FragmentSignup;
...
public class SignUpActivity extends BaseFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		replaceFragement(FragmentSignup.newInstanse(extras),FragmentSignup.TAG);
	}
	...
	private void commonCallBack(Object object, int requestCode, Object fragment2) {
		...
		case Constants.signup:
			if (jObject.optInt(Constants.dataToFollow, Constants.NOT_FLOW) == Constants.FLOW) {
				((FragmentSignup)fragment2).setresponce(jObject);
			//	finish();
			} else {
				 showDialog(message);
			}
