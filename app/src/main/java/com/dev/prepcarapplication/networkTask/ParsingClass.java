...
public class ParsingClass  
{
		...
		{		
			...
			try 
			{
				...
				//	RetryPolicy policy = new DefaultRetryPolicy(socketTimeout, DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
					...
					//editing
					StringRequest req = new StringRequest(method, strUrl,
	...
	public static void printLongCat(String value) {
		int maxLogSize = 1000;
		for (int i = 0; i <= value.length() / maxLogSize; i++) {
			int start = i * maxLogSize;
			int end = (i + 1) * maxLogSize;
			end = end > value.length() ? value.length() : end;
			Log.v("Base String", value.substring(start, end));
		}
	}
