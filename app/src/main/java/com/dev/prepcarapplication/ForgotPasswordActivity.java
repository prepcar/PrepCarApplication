...
import com.dev.prepcarapplication.fragment.FragmentForgotPassword;
...
public class ForgotPasswordActivity extends BaseFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		replaceFragement(FragmentForgotPassword.newInstanse(extras),FragmentForgotPassword.TAG);
	}
	...
	private void commonCallBack(Object object, int requestCode, Object fragment2) {
		...
		case Constants.forgetPass:
			if (jObject.optInt(Constants.status, Constants.NOT_FLOW) == Constants.FLOW) {
				((FragmentForgotPassword)fragment2).setresponce(jObject);
			} else {
				 showDialog(message);
			}
			break;
