...
import com.dev.prepcarapplication.fragment.FragmentIAmHesitant;

public class IAmHesitantActivity extends BaseFragmentActivity {

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_container);
		replaceFragement(FragmentIAmHesitant.newInstanse(null),FragmentIAmHesitant.TAG);
	}

	@Override
	public boolean onCreateOptionsMenu(Menu menu) {
		// Inflate the menu; this adds items to the action bar if it is present.
		getMenuInflater().inflate(R.menu.iam_hesitant, menu);
		return true;
	}
	...
	private void commonCallBack(Object object, int requestCode, Object fragment2) {
		...
		switch (requestCode) {
		case Constants.getIAM:
			if (jObject.optInt(Constants.status, Constants.NOT_FLOW) == Constants.FLOW) {
				((FragmentIAmHesitant)fragment2).setresponce(jObject);
			} else {
				 showDialog(message);
			}
			break;
		case Constants.getnewcarsecond:
			if (jObject.optInt(Constants.status, Constants.NOT_FLOW) == Constants.FLOW) {
				((FragmentIAmHesitant)fragment2).setresponcefinal(jObject);
			} else {
				 showDialog(message);
			}
			break;
	...
	public void showDialog(String msg){
		final AlertDialog.Builder builder = new AlertDialog.Builder(IAmHesitantActivity.this);
