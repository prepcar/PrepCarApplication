...
public class BeforeLoginActivity extends BaseFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		replaceFragement(FragmentBeforeLogin.newInstanse(extras),FragmentBeforeLogin.TAG);
	...
	private void commonCallBack(Object object, int requestCode, Object fragment2) {
		...
		switch (requestCode) {
		case Constants.fbLogin:
			if (jObject.optInt(Constants.status, Constants.NOT_FLOW) == Constants.FLOW) {
				((FragmentBeforeLogin)fragment2).setresponce(jObject);
			} else {
				 showDialog(message);
			}
			break;
	...
	public void showDialog(String msg){
		final AlertDialog.Builder builder = new AlertDialog.Builder(BeforeLoginActivity.this);
