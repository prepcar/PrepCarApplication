...
import com.dev.prepcarapplication.fragment.FragmentDiscovery;
...
public class DiscoveryActivity extends BaseFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		replaceFragement(FragmentDiscovery.newInstanse(null),FragmentDiscovery.TAG);
	}
	...
	private void commonCallBack(Object object, int requestCode, Object fragment2) {
		...
		case Constants.discovery:
			if (jObject.optInt(Constants.status, Constants.NOT_FLOW) == Constants.FLOW) {
				((FragmentDiscovery)fragment2).setresponce(jObject);
			} else {
				 showDialog(message);
			}
			break;
		case Constants.getDiscovery:
			if (jObject.optInt(Constants.status, Constants.NOT_FLOW) == Constants.FLOW) {
				((FragmentDiscovery)fragment2).setresponceDetail(jObject);
			} else {
				 showDialog(message);
			}
			break;
	...
	public void showDialog(String msg){
		final AlertDialog.Builder builder = new AlertDialog.Builder(DiscoveryActivity.this);
